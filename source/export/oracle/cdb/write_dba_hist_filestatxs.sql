...
-- 
-- Select data from DBA_HIST_FILESTATXS into CSV file.
-- 
...
spool &OUT_DIR./hist_filestat.csv
...
select a.snap_id, a.dbid, a.instance_number, b.startup_time,
  ...
  a.phyrds - lag(a.phyrds)
    ...
    a.file#, a.creation_change# order by a.snap_id) dif_phyrds,
  a.phywrts old_phywrts,
  a.phywrts - lag(a.phywrts)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_phywrts,
  a.singleblkrds old_singleblkrds,
  a.singleblkrds - lag(a.singleblkrds)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_singleblkrds, 
  a.readtim old_readtim,
  a.readtim - lag(a.readtim)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_readtim, 
  a.writetim old_writetim, 
  a.writetim - lag(a.writetim)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_writetim, 
  a.singleblkrdtim old_singleblkrdtim, 
  a.singleblkrdtim - lag(a.singleblkrdtim)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_singleblkrdtim, 
  a.phyblkrd old_phyblkrd,
  a.phyblkrd - lag(a.phyblkrd)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_phyblkrd,
  a.phyblkwrt old_phyblkwrt,
  a.phyblkwrt - lag(a.phyblkwrt)
    over(partition by a.dbid, a.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_phyblkwrt, 
  b.end_interval_time,
  b.end_interval_time - lag(b.end_interval_time)
    over (partition by b.dbid, b.instance_number, b.startup_time, a.con_dbid,
    a.file#, a.creation_change# order by a.snap_id) dif_ivl,
